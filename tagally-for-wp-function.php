...
?>
		<ul class="subsub">
			<li><a href='?page=tagally-for-wp-function' <?php if($_GET['subpage'] == '') {print "class=current";} ?>> <?php print(_e("Options")); ?></a></li>
			<li><a href='?page=tagally-for-wp-function&subpage=manage' <?php if($_GET['subpage'] == 'manage') echo "class=current"; ?>><?php print(_e("Manage"));?></a></li>
		</ul>
<?php
	...
	function tag_manage_panel()
	{
		...
		if ($_POST["export"] == "completed")
		{
			...
			if ($this->tagsys)
			{
				...
				from $wpdb->posts p
				inner join $wpdb->term_relationships tr on p.id = tr.object_id and p.post_status = 'publish' and p.post_type = 'post'
...
SQL;
			...
			{
				$sql = "select p.id as id,p.post_title as post_title,p.post_content as post_content,p.guid as guid,p.post_date as post_date,t.tag as tag from $wpdb->posts p inner join $this->tbpost2tag pt on pt.post_id = p.id and p.post_status = 'publish' and p.post_type = 'post' inner join $this->tbtags t on t.tag_id = pt.tag_id";
			}
		  ...
		  if($posts)
			{
				...
				foreach($posts as $p)
				{
					...
					$excerpt = $this->cut_str(strip_tags($p->post_content),250,"UTF-8");
				...
				if ($fp)
				{
					...
					echo "<div id=\"message\" class=\"updated fade\"><p><strong>Export posts and related tags completed.</strong></p></div>";
				}
				else
				{
						echo "<div id=\"message\" class=\"updated fade\"><p><strong>Can't connetct to www.tagally.com.</strong></p></div>";
				}
			}
			else
			{
				echo "<div id=\"message\" class=\"updated fade\"><p><strong>No any posts exported.</strong></p></div>";
			}
		}
		...
		echo <<< HTML
			<div class="wrap">
				...
				<a href="$this->siteurl/wp-content/plugins$this->install_directory/tagally-for-wp-help.html" target="_new"><b>Help!</b></a>
				<div class="narrow">
					<p>If you are using wordpress 2.3 or higher, or your blog version is lower than wordpress 2.3 but you have used UltimateTagWarrior plugins before, you can use the one-click method to export your posts and related tags.</p>
					...
					<form action="" method="post">
						<p class="submit">
							<input type="submit" name="submit" value="Export posts and related tags " />
							<input type="hidden" name="export" value="completed" />
						</p>
					</form>
				</div>
			</div>
				...
				<form method="post"> 
					<table class="tga-table" border="0" cellpadding="3" cellspacing="0">
						...
						<tr>
							...
							<td width="710"><select name="showpopuptooltips" id="showpopuptooltips">
HTML;

	 	echo "<option value=\"Yes\"";
   	if ($showpopuptooltips == "Yes")
   	{
   	...
   	echo <<< HTML
						...
						<tr>
							...
							<td width="710"><input name="mpfont" class="tga-input" size="2" maxlength="2" value="$mpfont">px</td>
						</tr>
						<tr>
							<th width="270" align="left" scope="row">Least popular Font:</th>
							<td width="710"><input name="lpfont" class="tga-input" size="2" maxlength="2" value="$lpfont">px</td>
						</tr>
						<tr>
							<th width="270" align="left" scope="row">TagCloud Title: <span style="font-size:10px;font-weight:normal">( max 40 letters )</span></th>
							<td width="710"><input type='text' name="cloudtitle" class="tga-input" size="40" maxlength="40" value="$title"></td>
						</tr>
						<tr>
							<th width="270" align="left" scope="row">Showing how many tags in tagcloud most: <span style="font-size:10px;font-weight:normal">  &nbsp;  ( max 99 tags)</span></th>
							<td width="710"><input type='text' name="maxtagcount" class="tga-input" size="2" maxlength="2" onkeyup="value=value.replace(/[^0-9]/, '')" value="$maxtagcount"></td>
						</tr>
						<tr>
							<td width="270" valign="top" rowspan="2">
							  <p>For example:</p>
								<div class="tagally_tagdiv" id="tga_exmdiv" style="background-color:#$prebgcolor;">
HTML;
...
HTML;
		...
		echo <<< HTML
									...
									<div id="tagally_vcd_0"  style="display:$display">
										<p id="tagally_ctp">
											How many colors : <INPUT TYPE="text" name="colorcount" class="tga-input" VALUE="$colorcount" id="tagallytbnum" size="3" maxlength="2" > 
...
HTML;
	  if(is_string($customcolor) && $customcolor!="")
	  {
		...
		echo <<< HTML
										...
										<p><input type="button" class="tga-button" id="tagally_apc_btn" value="Preview" onclick="apply_colortb()" style="float:left;display:none" /><INPUT TYPE="button" VALUE="Reset" class="tga-button" id="tagallyrscbtn" onclick="tga_createCoTb()" style="display:none" /></p>
										
										<input type="hidden" id="tagally_hdn_scot" name="tagally_hdn_scot" value="$customcolor">
										<input type="hidden" id="tagally_hdn_selno" name="tagally_hdn_selno" value="$colortype">
									</div>
...
HTML;
		...
		echo <<< HTML
							    ...
							    <div id="tagally_vcd_2"  style="display:$display">
										<p id="tagally_ctp_1">
...
HTML;
		...
		echo <<< HTML
						...
						<tr>
						<td  width="710"  >
						    <p><label class="tga-hlab">TagCloud sample background color: </label>&nbsp;&nbsp;&nbsp;&nbsp;#<input id="tga_exbg" name="prebgcolor" class="tga-input" type="text" size="6" maxlength="6" onblur="iupcolor(this , 'tga_exbgbtn')" onkeyup="value=value.replace(/[^0-9A-Fa-f]/, '')" value="$prebgcolor" /><input id='tga_exbgbtn' type='button' class='colourPickerButton'  value='' style='background-color:#$prebgcolor' />
	...
	function edit_post_tags()
	{
		...
		echo <<< HTML
		  	...
		  	<div class="dbx-content">
		  		<input id="tlmiftaglist" name="taglist" value="$taglist" style="width:98%" onfocus=" document.getElementById('hiddenTagList').style.display='block'" /><br />
		  	</div>
HTML;
	...
	function save_post_tags($postid)
	{
		...
		$sql = "select * from $wpdb->posts p where p.id = $postid and p.post_status = \"publish\" and p.post_type = \"post\"";
		$posts = $wpdb->get_results($sql);
		if($posts)
		{
			$p = $posts[0];
...
//				trigger_error("connect failed");
	...
	function admin_head_add_javascript()
	{
		if(!$this->tagsys)
		{
			$addtagjs = "$this->siteurl/wp-content/plugins$this->install_directory/inserttag-js.php";
			echo "<script src=\"$addtagjs\" type=\"text/javascript\"></script>";
		}
		if($_GET['page'] == 'tagally-for-wp-function' && $_GET['subpage'] != 'manage')
		{
			$cpjs = "$this->siteurl/wp-content/plugins$this->install_directory/colorpicker-js.php";
			$cpcs = "$this->siteurl/wp-content/plugins$this->install_directory/colorpicker.css";
	...
	{
		$shinejs = "$this->siteurl/wp-content/plugins$this->install_directory/uletsshine-js.php";
		$flcs = "$this->siteurl/wp-content/plugins$this->install_directory/fl.css";
	...
	function wp_footer_add_dropmenu()
	{
			echo <<<DROPMENU
	    		...
	    		<div id="tga_dpmtap" style="width:262px;height:16px"></div>
	    		<div class="dpms0" id="tga_dpms_0" style="background-color:#FFF;border-bottom:#898C95 1px solid;border-left:#898C95 1px solid;border-right:#898C95 1px solid;font-size:12px;margin:0px;width:260px;z-index:9999">	
	    			<form onsubmit="" action="" method="post" target="_blank">
	    	            <div class="dpms1">
	    	            	...
	    	            	<a id="tga_tNYIMU" href="http://www.tagally.com/main/addfavorite?tags="><img id="addlist" width="124" height="17"  src="$this->siteurl/wp-content/plugins$this->install_directory/addto.gif" style="padding:2px 0px 0px 0px;margin-top:2px;background:none;border-color:#FFF;border-style:none;border-width:0px;" alt="add this tag to your favorites in TagALLY.com"/></a>
	    	            </div>
	    	            ...
	    	            <div id="tga_liDv" style="border:0px;margin:3px 115px 10px;"><img style="max-width:none;background:none;border:0px none #FFFFFF;padding:0px;margin:0px" width="33" height="33" src="$this->siteurl/wp-content/plugins$this->install_directory/31.gif"/></div>
...
DROPMENU;
    ...
    while ($i <= strlen($msg))
    {
      //if it is ASCII extended charater
      if(ord($msg[($i-1)])>127)
      {
        $chinese++;
        //if $charset is UTF-8, skip three ASCII
        if($charset=="UTF-8")
        {
          $str .= $msg[($i-1)].$msg[($i)].$msg[($i+1)];
          $i=$i+3;
        }
        else //else skip three ASCII
        {
          $str .= $msg[($i-1)].$msg[($i)];
          $i=$i+2;
        }
      ...
      if(($chinese+$eng)==$cut_size)
      	break;
    }
    
    $suffix = ($chinese+$eng)<$cut_size?"":$suffix;
    return $str.$suffix;
	}
